...
JnB::JnB() : x(100), y(100), speed(0.7), currentFrame(0), frameCount(8), pistolScale(0.03f), pistolDistance(50.0f), pistolAngle(10.0f), bulletSpeed(0.8), health(3) {
    ...
    for (int i = 0; i < 8; i++) {
        characterTextures[i].loadFromFile("image/character" + std::to_string(i + 1) + ".png");
        characterTexturesLeft[i].loadFromFile("image/character_left" + std::to_string(i + 1) + ".png");
    }

    character.setTexture(characterTextures[0]);
    frameWidth = characterTextures[0].getSize().x / frameCount;

    pistolTexture.loadFromFile("image/pistol.png");

    pistolLeftTexture.loadFromFile("image/pistol_left.png");
...
void JnB::update(const std::vector<std::vector<int>>& collisionMap, sf::RenderWindow& window) {
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left) && ( collisionMap[y+30][x - 1-8] != 1)) {
        x -= speed;
        frameWidth = characterTexturesLeft[0].getSize().x / frameCount;
        character.setTexture(characterTexturesLeft[currentFrame]);
    } if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right) && ( collisionMap[y+30][x + 1+16] != 1))  {
        x += speed;
        frameWidth = characterTextures[0].getSize().x / frameCount;
        character.setTexture(characterTextures[currentFrame]);
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up) && ( collisionMap[y+16][x+8] != 1)) {
        y -= speed;
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down) && ( collisionMap[y +1+32][x+16] != 1)) {
        y += speed;
    }
...
void JnB::resizeCharacter(float scaleX, float scaleY) {
